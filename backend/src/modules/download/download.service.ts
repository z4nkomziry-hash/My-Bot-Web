...
export class DownloadService {
  ...
  /**
   * Add download to queue for background processing
   */
  async addToQueue(url: string, quality?: string, userId?: string) {
    ...
    return { jobId: job.id, status: 'queued', message: 'داونلۆدەکەت خرایە ڕیز' };
  }
  ...
  async extractAudio(url: string) {
    ...
    if (platform === 'TikTok') {
      ...
      if (result.audioUrl) return { audioUrl: result.audioUrl, platform };
    }
    ...
    return { audioUrl: response.data.url, platform };
  }
  ...
  async getThumbnail(url: string) {
    ...
    if (platform === 'TikTok') {
      const result = await this.downloadTikTok(url);
      if (result.cover) return { thumbnailUrl: result.cover, platform };
    }
  ...
  /**
   * Download from other platforms using Cobalt API
   */
   ...
   */
  private async saveToHistory(url: string, platform: string, quality?: string, userId?: string) {
